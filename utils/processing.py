...
def process_flood_damage(crop_raster_path, depth_raster_paths, output_dir, period_years, samples, crop_inputs, flood_metadata):
    ...
    for depth_path in depth_raster_paths:
        ...
        for crop_code, props in crop_inputs.items():
            ...
            # Simulate direct damage with samples (but not Monte Carlo)
            sampled_damages = []
            for _ in range(samples):
                ...
                sampled_damages.append(total_loss)

            mean_direct = np.mean(sampled_damages)
            p5 = np.percentile(sampled_damages, 5)
            p95 = np.percentile(sampled_damages, 95)
            std = np.std(sampled_damages)
            ...
            summary_rows.append({
                ...
                "DirectDamage_Mean": round(mean_direct, 2),
                "DirectDamage_Std": round(std, 2),
                "Loss_5th": round(p5, 2),
                "Loss_95th": round(p95, 2),
                "DollarsLost": round(mean_direct, 2),
                "EAD": round(mean_direct * (1 / return_period), 2)
            })
        ...
        damage_raster_paths[label] = damage_output_path
    ...
    with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
        ...
        for flood in all_summaries:
            ...
            data = Reference(sheet, min_col=9, min_row=1, max_row=sheet.max_row)
    ...
    return excel_path, all_summaries, diagnostics, damage_raster_paths
