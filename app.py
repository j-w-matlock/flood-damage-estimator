...
for key in ["result_path", "summaries", "diagnostics", "crop_path", "depth_paths", "monte_carlo_summary"]:
...
if crop_file:
    ...
    for code in most_common:
        ...
        months = st.multiselect(f"Crop {code} — Growing Season (months 1–12)", options=list(range(1, 13)),
    ...
    if not crop_file or not depth_files:
        ...
        with st.spinner("Processing flood damage estimates..."):
            ...
            try:
                ...
                result_path, summaries, diagnostics, *extras = results
                monte_carlo_summary = extras[0] if extras else None

                st.session_state.result_path = result_path
                st.session_state.summaries = summaries
                st.session_state.diagnostics = diagnostics
                st.session_state.monte_carlo_summary = monte_carlo_summary
...
if st.session_state.result_path and st.session_state.summaries:
    st.download_button("📅 Download Excel Summary",
                       data=open(st.session_state.result_path, "rb"),
...
if st.session_state.monte_carlo_summary:
    st.markdown("## 🎲 Monte Carlo Uncertainty Analysis")
    for flood, df_mc in st.session_state.monte_carlo_summary.items():
